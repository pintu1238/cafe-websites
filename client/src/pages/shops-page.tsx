...
const sortOptions = [{ value: 'popular', label: 'Popular' }, { value: 'rating', label: 'Top rated' }, { value: 'distance', label: 'Nearest' }, { value: 'preparation', label: 'Fastest prep' }, { value: 'newest', label: 'Newest' }];
const ratingOptions = [{ value: '', label: 'Any rating' }, { value: '4', label: '4.0+ stars' }, { value: '4.5', label: '4.5+ stars' }];
const distanceOptions = [{ value: '', label: 'Any distance' }, { value: '0.5', label: 'Within 0.5 km' }, { value: '1', label: 'Within 1 km' }, { value: '2', label: 'Within 2 km' }];
const priceOptions = [{ value: '', label: 'Any price' }, { value: 'BUDGET', label: 'Budget · ₹' }, { value: 'MID', label: 'Mid-range · ₹₹' }, { value: 'PREMIUM', label: 'Premium · ₹₹₹' }] as const;
  ...
  return <div className="discovery-page section-shell py-10 sm:py-14">
    ...
    <section className="mt-8 rounded-[1.75rem] border border-espresso/10 bg-white p-4 shadow-card sm:p-5" aria-label="Cafeteria filters">
      <div className="discovery-toolbar">
        ...
        <div className="discovery-sort-control"><SlidersHorizontal size={15} /><DropdownSelect aria-label="Sort cafeterias" value={sort ?? 'popular'} onChange={(value) => setSort(value as ShopFilters['sort'])} options={sortOptions} className="discovery-sort-select" /></div>
      ...
      {filtersOpen && <div className="discovery-filter-drawer mt-4 border-t border-espresso/10 pt-4"><div className="discovery-filter-field"><span>Minimum rating</span><DropdownSelect aria-label="Minimum rating" value={minRating} onChange={setMinRating} options={ratingOptions} /></div><div className="discovery-filter-field"><span>Distance</span><DropdownSelect aria-label="Distance" value={maxDistanceKm} onChange={setMaxDistanceKm} options={distanceOptions} /></div><div className="discovery-filter-field"><span>Price range</span><DropdownSelect aria-label="Price range" value={priceRange ?? ''} onChange={(value) => setPriceRange((value || undefined) as ShopFilters['priceRange'])} options={priceOptions} /></div><label className="flex items-center gap-2 self-end rounded-xl border border-espresso/10 px-3 py-2 text-sm font-semibold text-espresso"><input type="checkbox" checked={offersOnly} onChange={(event) => setOffersOnly(event.target.checked)} className="h-4 w-4 accent-ember" />Offers only</label><button type="button" className="button-secondary self-end" onClick={clearFilters}><X size={15} />Clear filters</button></div>}
    </section>

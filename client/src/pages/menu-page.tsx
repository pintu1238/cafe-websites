...
export function MenuPage() {
  ...
  return <PortalShell eyebrow="The campus menu" title="What sounds good today?" intro="Choose a cafeteria, find your favourites, and build your next campus meal." action={{ label: 'Browse all cafeterias', to: '/shops' }}>
    <div className="portal-menu-toolbar"><label>Cafeteria<select value={slug} onChange={(event) => update('cafeteria', event.target.value)} disabled={shops.isLoading}><option value="" disabled>Select a cafeteria</option>{shops.data?.items.map((shop) => <option key={shop.id} value={shop.slug}>{shop.name}{shop.isOpen ? '' : ' · Closed'}</option>)}</select></label><label className="portal-search"><Search size={19} /><span className="sr-only">Search menu</span><input type="search" maxLength={100} placeholder="Search dishes or ingredients…" value={query} onChange={(event) => update('q', event.target.value)} /></label><label className="portal-checkbox"><input type="checkbox" checked={vegetarian} onChange={(event) => update('veg', event.target.checked ? '1' : '')} />Vegetarian only</label></div>

...
export function BrandMark({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  return <span className={cn('inline-flex items-center gap-2.5', light ? 'text-white' : 'text-espresso')}>
    ...
    <span className="brand-mark-copy leading-none"><span className="block text-lg font-extrabold tracking-tight">UniEats</span><span className={cn('brand-mark-tagline mt-1 block text-[9px] font-medium tracking-[0.08em]', light ? 'text-white/60' : 'text-muted')}>Good Food. Greater Together.</span></span>
  </span>;
}
